...
public class MinioStorageService : IStorageService
{
    ...
    public MinioStorageService(IConfiguration config)
    {
        ...
        var publicEndpoint = config["Minio:PublicEndpoint"] ?? endpoint;
        var access   = config["Minio:AccessKey"]!;
        var secret   = config["Minio:SecretKey"]!;
        _bucket      = config["Minio:Bucket"]!;

        var scheme = publicEndpoint.Contains("localhost") ? "http" : "https";
        _publicBase  = $"{scheme}://{publicEndpoint}/{_bucket}";
